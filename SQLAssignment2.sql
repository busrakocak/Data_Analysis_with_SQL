...
CREATE TABLE Actions(Visitor_ID int, Adv_Type VARCHAR(15), Action_ VARCHAR(15))
...
SELECT Adv_Type,COUNT(Action_) as number_action, SUM(CASE When Action_ = 'Order' THEN 1 ELSE 0 END ) as number_order 
FROM Actions
WHERE Adv_Type = 'A'
GROUP BY Adv_Type
...
SELECT Adv_Type, ROUND(CAST(number_order as float)/CAST(number_action as float),2)  AS Conver_Rate
FROM
    (SELECT  Adv_Type, COUNT(Action_) as number_action, SUM(CASE When Action_ = 'Order' THEN 1 ELSE 0 END ) as number_order 
	FROM Actions
	WHERE Adv_Type = 'A'
...
UNION
	...
	SELECT  Adv_Type, COUNT(Action_) as num_action, SUM(CASE When Action_ = 'Order' THEN 1 ELSE 0 END ) as number_order 
	FROM Actions
	WHERE Adv_Type = 'B'
	GROUP BY Adv_Type
) the_table